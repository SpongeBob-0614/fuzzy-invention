...
```text
requirements.yaml → 模型生成结构化计划 → 模型生成权限函数 → 手动运行测试
```

- `tasks/github-spreadsheets/requirements.yaml`：第一课的精简需求输入。
- `main.py`：读取 YAML，打印模块，为第一个模块生成计划，校验后保存到 `runs/permissions-plan.json`。
- `generate_permissions.py`：读取计划和测试，生成 `change_permission()`，检查 Python 语法后保存到 `runs/generated/permissions.py`。
...
查看生成的 `runs/generated/permissions.py` 后，运行测试：

```sh
PYTHONPATH=runs/generated .venv/bin/python tests/test_permissions.py
```

前两个脚本各调用一次模型，测试不调用模型。测试通过时输出 `两个权限场景均通过`。
`runs/` 中的计划和代码是本地生成产物，不提交到 Git；首次使用需要先生成，再运行测试。
